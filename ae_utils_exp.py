...
def calculate_auroc(targ, targ_ind, lat_ind, z, _ma, _mi, stepsize=0.01):
...
def aurocs_search(dl, ae):
    aurocs_all = torch.ones((40, ae.z_dim)) * 0.5
    with torch.no_grad():
        ...
        out = ae(data)
        _ma = ae.z.max(dim=0)[0]
        _mi = ae.z.min(dim=0)[0]
        if type(ae) is VAE:
            _ma = ae.mu.max(dim=0)[0]
            _mi = ae.mu.min(dim=0)[0]
        for i in range(40):
            print(i)
            aurocs_all[i] = aurocs(ae.z.clone(), targ, i, _ma, _mi)
    return aurocs_all.cpu(), base_rates_all.cpu(), targ.cpu()
...
from architectures import FactorVAE_Discriminator
    
class FACTOR_VAE(VAE):
    
    def __init__(self, inp_norm, enc, dec, device, z_dim, inp_inorm, rec_dstr='gaussian'):
        super(FACTOR_VAE, self).__init__(inp_norm, enc, dec, device, z_dim, inp_inorm, rec_dstr)
        self.disc = FactorVAE_Discriminator(z_dim, 1024)
        self.disc = self.disc.to(device)
        
    def init_optim_objects(self, lr):
        self.ae_optim = torch.optim.Adam(list(self.enc.parameters()) + list(self.dec.parameters()), lr=lr)
        self.disc_optim = torch.optim.Adam(self.disc.parameters(), lr=0.0001, betas=(0.5, 0.9))
    ...
    def fit(self, dataset, n_group, batch_per_group=10, lr=0.001, beta=1.0,\
        ...
        kl_loss = torch.zeros(n_group)
        # define samplers for the AE
        n_samples = batch_size*batch_per_group*n_group*2
        random_sampler_ae = torch.utils.data.RandomSampler(dataset, \
                      replacement=True, num_samples=n_samples, generator=generator_ae)
        batch_sampler_ae = torch.utils.data.BatchSampler(random_sampler_ae, batch_size=batch_size, drop_last=False)
        dataloader_ae = iter(torch.utils.data.DataLoader(dataset, batch_sampler=batch_sampler_ae))
        ...
        for g in range(n_group):
            ...
            for b in range(batch_per_group):
                ...
                out = self.forward(ex)
                z = self.z.detach()
                z_fake = fake_sample(z, self.device)
                
                d_pred_real = self.disc(z) # should be (batch_size,)
                d_pred_fake = self.disc(z_fake)
                disc_loss = 0.5*(self.bce_criterion(d_pred_real, torch.ones_like(d_pred_real)) + \
                               self.bce_criterion(d_pred_fake, torch.zeros_like(d_pred_fake)))
                disc_loss.backward() # maximize the Discriminator criterion
                ...
                # compute the KL divergence between the encoding distribution and the prior distribution
                kl_loss_b = compute_kl_div(self.z, self.mu, torch.exp(0.5*self.log_var)).mean(dim=0).sum()
                # add extra TC term via discriminator
                d_pred_real = self.disc(self.z)
                tc_loss = (d_pred_real/(1. - d_pred_real + _fvae_eps)).log().mean() # empirical TC. want to minimize this
                
                rec_loss_agg += rec_loss_b.item()
                kl_loss_agg += kl_loss_b.item()
                (rec_loss_b + kl_loss_b + beta*tc_loss).backward()
                self.ae_optim.step()
            rec_loss[g] = rec_loss_agg / batch_per_group
            kl_loss[g] = kl_loss_agg / batch_per_group
            print("\tRec: {:1.4f}\tKL: {:1.4f}".format(\
